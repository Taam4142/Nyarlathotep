...
| R13 | **npm audit** — as of 2026-08-18: **8 advisories (1 critical, 2 high, 5 moderate)**, up from the 2 moderate recorded on 2026-08-07. | Low | **The rise was not caused by any change here** — verified by running `npm audit` against the previous commit's lockfile, which reports the identical 8. The advisory database simply moved in the intervening days. **7 of the 8 are genuinely dev-only** (`vitest`, `@vitest/mocker`, `vite`, `vite-node`, `esbuild`, `nanoid` — the test/build toolchain). The critical one is `vitest`: *arbitrary file read when the Vitest UI server is listening*. This project never starts the Vitest UI (`npm run test` is `vitest run`), and it is a dev dependency, so it does not ship. ⚠️ **Correction to what this row previously claimed.** It said "neither reaches the browser bundle". That is **wrong for `uuid`**: `exceljs` is a **runtime** dependency (in `dependencies`, dynamic-imported into the shipped `xlsx-*.js` chunk), and `uuid` is bundled with it — confirmed by grepping the built asset. The *conclusion* still holds, but for a different reason: the advisory is *Missing buffer bounds check in v3/v5/v6 when `buf` is provided*, and exceljs imports **only `v4`** (`const {v4: uuidv4} = require('uuid')`, in `cf-rule-ext-xform.js`) with no `buf` argument. The vulnerable functions are present in the bundle but never invoked. Recorded precisely because the old reasoning was reusable and wrong — anyone leaning on "exceljs is dev-only" to wave off a *future* exceljs advisory would be mistaken. | Fixing the dev-only ones needs breaking major bumps (vitest 4.x / vite 6.x→7.x). Deliberately **not** done silently — engineer's call. For `uuid`, the fix is an `exceljs` major bump; low priority while the vulnerable path stays unreachable, but **re-check on any exceljs upgrade or if exceljs starts using v3/v5/v6**. Re-run `npm audit` when either package moves for other reasons. |
