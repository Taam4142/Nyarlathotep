...
async function ocrAllPages(pdfFile, googleKey, projectId, processorId) {
  ...
  for (const pageBase64 of pages) {
    const text = await callDocumentAI(pageBase64, googleKey, projectId, processorId);
    texts.push(text);
  }
  return texts.join('\n\n--- PAGE BREAK ---\n\n');
}
...
.thai-text { font-family: 'Sarabun', sans-serif; line-height: 1.8; }
```
...
1. **Tesseract.js removed** — replaced by Google Document AI for scanned PDFs. Tesseract Thai accuracy was insufficient for professional bid documents.
